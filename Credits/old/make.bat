...
powerpc-gekko-gcc-3.4.4 -nostdlib -fno-exceptions -Os  -mhard-float -Wl,--gc-sections -Wl,-T -Wl,first.lds -Wl,-Map,Bin/%PROGNAME%.map boot.s ray.c debug.c -o %BINDIR%/%PROGNAME%.elf 
powerpc-gekko-objcopy -O binary %BINDIR%/%PROGNAME%.elf %BINDIR%/%PROGNAME%.bin

@rem bin2dol %BINDIR%/%PROGNAME%.bin %BINDIR%/%PROGNAME%2.dol >> NUL    -maltivec 
